...
export function AudioVisualizerBackground() {
  ...
  const { frequencyBars, isActive } = useAudioAnalyser(BAR_COUNT);
  const frequencyBarsRef = useRef(frequencyBars);
  const isActiveRef = useRef(isActive);

  // Keep refs in sync
  useEffect(() => {
    frequencyBarsRef.current = frequencyBars;
    isActiveRef.current = isActive;
  }, [frequencyBars, isActive]);
  ...
  useEffect(() => {
    ...
    // Generate smooth noise-like values for idle animation
    const generateIdleValues = (time: number): number[] => {
      const values: number[] = [];
    ...
    const draw = (time: number) => {
      ...
      // Use real audio data if active, otherwise use idle animation
      const values = isActiveRef.current 
        ? frequencyBarsRef.current 
        : generateIdleValues(time);
      ...
      values.forEach((value, i) => {
        const height = Math.max(value, 0.02) * maxHeight;
        const x = i * barWidth;
        const y = canvas.height - height;

        const gradient = ctx.createLinearGradient(x, canvas.height, x, y);
        
        // More vibrant colors when active
        if (isActiveRef.current) {
          gradient.addColorStop(0, "hsla(145, 75%, 50%, 0.8)");
          gradient.addColorStop(0.5, "hsla(270, 70%, 60%, 0.6)");
          gradient.addColorStop(1, "hsla(270, 70%, 60%, 0.2)");
        } else {
      ...
      values.forEach((value, i) => {
        const height = Math.max(value, 0.02) * maxHeight * 0.3;
        const x = i * barWidth;
        const barWidth_ = canvas.width / BAR_COUNT;

        const gradient = ctx.createLinearGradient(x, 0, x, height);
        gradient.addColorStop(0, isActiveRef.current 
          ? "hsla(145, 75%, 50%, 0.3)" 
          : "hsla(145, 65%, 42%, 0.2)");
        gradient.addColorStop(1, "hsla(145, 65%, 42%, 0)");

        ctx.fillStyle = gradient;
        ctx.fillRect(x + 1, 0, barWidth_ - 2, height);
      });
